...
export class CompositorService {
  ...
  async captureCompositorData (browser, name) {
    try {
      ...
      const canvasHash = await this.getCanvasHash(page)
      ...
      const finalHash = hash + canvasHash
      console.log(`Final: Captured ${processedCommands.length} paint commands, hash: ${finalHash}`)
    ...
    const canvasData = await page.evaluate(() => {
      ...
      canvases.forEach((canvas, index) => {
        try {
          ...
          canvasData += `canvas${index}:${dataUrl};`
        } catch (e) {
          console.log(`Canvas ${index}: Cannot read data (${e.message})`)
          // Ignore canvases that cannot be read
        }
      })
      return canvasData
    })
