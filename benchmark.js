...
function parseCompositorOutput(output) {
	...
	const actualMatch = output.match(/Actual:   ([a-f0-9]+)/)
...
async function runBenchmark() {
	...
	for (let i = 0; i < config.iterations; i++) {
		...
		try {
			const { output, duration } = await runScript('capture-compositor.js')
	...
	for (let i = 0; i < config.iterations; i++) {
		...
		try {
			const { output, duration } = await runScript('capture-pixels.js')
